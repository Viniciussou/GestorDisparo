...
export function DispatchPanel({ showToast }: DispatchPanelProps) {
    ...
    try {
      ...
      if (data.success) {
        if (data.debug) {
          ...
          showToast('QR Code simulado gerado para debug. Use um número diferente que não esteja banido.', 'error')
        } else if (data.qr) {
          ...
          // Aguardar conexão - verificar status periodicamente
          const checkConnection = setInterval(async () => {
            try {
              ...
              if (statusData.success && !statusData.qr) {
                // QR code foi escaneado, verificar se conectou
                const sessionResponse = await fetch('/api/whatsapp?action=status')
                const sessionData = await sessionResponse.json()
                
                const session = sessionData.sessions?.find((s: any) => s.phone === phone)
                if (session?.connected) {
                  ...
                  showToast(`Número ${formatPhone(phone)} conectado com sucesso!`, 'success')
                }
              }
            } catch (error) {
          ...
          setTimeout(() => {
            clearInterval(checkConnection)
            if (connectingPhone === phone) {
              setQrCode(null)
              setQrImage(null)
              setConnectingPhone(null)
              showToast('Timeout: Tente conectar novamente.', 'error')
            }
          }, 300000)
        ...
        // Tratar erro específico de número banido
        if (data.banned) {
          showToast(data.error || 'Número banido pelo WhatsApp', 'error')
          // Mostrar sugestões no console
          if (data.suggestions) {
  ...
  const executeDispatchNow = useCallback(async () => {
    ...
    const currentSender = connectedSenders[currentSenderIndex % connectedSenders.length]
  ...
  }, [dispatchConfig.isActive, countdown > 0])
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      {/* QR Code Modal */}
      <Dialog open={!!qrImage && !!connectingPhone} onOpenChange={(open) => {
        if (!open) {
          setQrCode(null)
          setQrImage(null)
          setConnectingPhone(null)
              ...
              {isDebugMode ? (
                <div className="w-64 h-64 border border-red-500 rounded-lg flex items-center justify-center bg-red-50">
                  <div className="flex flex-col items-center gap-2 text-red-600">
                    ...
                    <p className="text-xs text-center">Este QR é apenas para debug.<br/>Use um número não banido.</p>
                  </div>
                </div>
              ) : qrImage ? (
                <img 
      ...
      <div className={`p-6 rounded-lg border ${
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className={`w-12 h-12 rounded-full flex items-center justify-center ${
              dispatchConfig.isActive ? 'bg-green-500/20' : 'bg-muted'
            }`}>
              {dispatchConfig.isActive ? (
        ...
        {dispatchConfig.isActive && (
          <div className="mt-4 pt-4 border-t border-border/50 grid grid-cols-4 gap-4">
            ...
            <div className="text-center">
              <p className="text-2xl font-bold">{connectedSenders[currentSenderIndex % connectedSenders.length]?.name || '-'}</p>
                ...
                {INTERVAL_OPTIONS.map((option) => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-md text-sm border transition-colors ${
                      dispatchConfig.interval === option.value
